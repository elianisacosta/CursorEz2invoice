-- Mechanics Management Database Schema
...
CREATE POLICY "Users can view their own mechanics" ON mechanics
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own mechanics" ON mechanics
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own mechanics" ON mechanics
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own mechanics" ON mechanics
    FOR DELETE USING (auth.uid() = user_id);

-- Policies for timesheets table
CREATE POLICY "Users can view timesheets for their mechanics" ON timesheets
    FOR SELECT USING (
        mechanic_id IN (
...
CREATE POLICY "Users can insert timesheets for their mechanics" ON timesheets
...
CREATE POLICY "Users can update timesheets for their mechanics" ON timesheets
    FOR UPDATE USING (
        mechanic_id IN (
            SELECT id FROM mechanics WHERE user_id = auth.uid()
        )
...
CREATE POLICY "Users can delete timesheets for their mechanics" ON timesheets
    FOR DELETE USING (
        mechanic_id IN (
            SELECT id FROM mechanics WHERE user_id = auth.uid()
        )
